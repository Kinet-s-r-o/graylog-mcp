...
async function loadSaved(){let r=await fetch('/ui/api/queries'),d=await r.json();queryItems=d.queries||[];$('saved').innerHTML=queryItems.map(x=>`<option value="${uiEscape(x.name)}">${uiEscape(x.name)} — ${uiEscape(x.description||'')}</option>`).join('');renderQueries()}
